...
static int translate_readonly(const char *file)
{
	...
	if (class_fd < 0) {
            warn("no remoteproc class, falling back to flat firmware path, file : %s", file);

        /* try sysfs fw path directly */
            if (strlen(fw_sysfs_path) > 0 &&
              	strlen(fw_sysfs_path) + 1 + strlen(file) + 1 < sizeof(path)) {
        ...
        /* try FIRMWARE_BASE directly */
            if (strlen(FIRMWARE_BASE) + strlen(file) + 1 < sizeof(path)) {
